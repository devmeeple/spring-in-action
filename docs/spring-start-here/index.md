...
- 구현 시 직접적으로 결합하면 안 되는 이유와 사례, 인터페이스 도입
- POJO는 의존성이 없는 단순 객체를 의미한다. 속성과 메서드만 포함한다.
- 스트레오 타입 어노테이션 사용 위치, 인터페이스는 인스턴스화할 수 없으므로 추가해도 의미 없다.
- DI를 왜 선택해야 하는가? 스프링에서 DI를 선택한 이유.
- 구체적인 스트레오타입 어노테이션
