...
- 단위 테스트는 테스트하는 기능의 모든 의존성(데이터베이스, 외부 API, 다른 서비스 계층 등)을 제거하고 격리하여 순수한 로직을 테스트한다.
  의존성으로 테스트 결과가 달라지거나 속도가 느려지는 것을 방지한다.
- 단위 테스트를 격리하기 위해 모킹(Mocking)을 사용한다.

**Mocking**

- Mock은 테스트 대상이 의존하는 객체를 흉내 내는 가짜 객체다.
- Mock 객체는 실제 객체처럼 작동하는 대신, 특정 입력이 주어졌을 때 미리 정해진 출력(반환 값)을 돌려주도록 설정한다.
- Mockito 기능(`@Mock`, `given`, `verify`)은 순수 자바 단위 테스트에 사용한다.
- Stubbing: `given()` 또는 `when().thenReturn()`: Mock 객체의 특정 메서드가 특정 인자와 함께 호출되면, 해당 값을 반환하라.
- `verify()`: Mock 객체의 특정 메서드가 예상대로 호출되었는지 확인(몇 번, 어떤 인자로)
- 스프링 테스트 환경에서는 객체를 주입하고 관리하는 방식에 따라 `@InjectMocks`, `@MockBean`을 사용한다. 
