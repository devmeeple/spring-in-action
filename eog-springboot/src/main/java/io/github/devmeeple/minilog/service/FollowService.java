...
import io.github.devmeeple.minilog.dto.FollowResponseDto;
import io.github.devmeeple.minilog.entity.Follow;
import io.github.devmeeple.minilog.entity.User;
import io.github.devmeeple.minilog.exception.UserNotFoundException;
import io.github.devmeeple.minilog.repository.FollowRepository;
...
public class FollowService {
    ...
    public FollowResponseDto follow(Long followerId, Long followeeId) {
        ...
        User follower = userRepository.findById(followerId).orElseThrow(
                () -> new UserNotFoundException(String.format("팔로워 아이디(%d)를 가진 사용자를 찾을 수 없습니다.", followerId))
        );
        User followee = userRepository.findById(followeeId).orElseThrow(
                () -> new UserNotFoundException(String.format("팔로잉 아이디(%d)를 가진 사용자를 찾을 수 없습니다.", followeeId))
        );
    ...
    public void unfollow(Long followerId, Long followeeId) {
        Optional<Follow> follow = Optional.ofNullable(followRepository.findByFollowerIdAndFolloweeId(followerId, followeeId).orElseThrow(
                () -> new UserNotFoundException(String.format("팔로워(%d)와 팔로잉(%d)를 연결하는 Follow를 찾을 수 없습니다.", followerId, followeeId))
        ));
